...
| `redis`     | `redis:7-alpine`               | —           | Object cache backend (Redis Object Cache plugin)   |
| `wordpress` | custom (`wordpress:6.9-php8.3-fpm` + redis/intl/zip) | — | PHP-FPM running WordPress |
...
- **Redis Object Cache** — the source relies on caching; wiring it now keeps parity and
  surfaces cache-related bugs early.
...
3. sets options + `/%postname%/` permalinks,
4. installs/activates plugins: Redis Object Cache, Rank Math SEO, WPForms Lite,
   Redirection, Safe SVG,
5. activates the `buckleup` theme,
6. runs the seed eval-files in `scripts/wp/`:
   ...
   - `seed-catalog.php` — 3 services + 2 packages,
   - `seed-content.php` — front page, core pages, 5 location pages, exemplar testimonial,
   - `import-media.php` — brand assets into the Media Library.

Re-run any time; everything upserts by slug/email/filename.
