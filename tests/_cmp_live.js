...
(async () => {
  const targets = { live:'https://www.buckleupdriving.ca', local:'http://localhost:8080' };
  ...
  for (const [name, base] of Object.entries(targets)) {
    for (const [vp, w] of [['d',1440],['m',390]]) {
      ...
      try {
        await p.goto(base+'/?cb='+Date.now(), { waitUntil:'networkidle', timeout:60000 });
        ...
        await p.screenshot({ path:`/tmp/cmp_${name}_${vp}.png`, fullPage:true });
